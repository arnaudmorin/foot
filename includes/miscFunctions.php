...
function createUser($login, $gift="", $password="", $number="") {
    ...
    if (!isset($res['login'])){
        ...
        $req="INSERT INTO members (login, number, gift, password) VALUES ('".mysql_real_escape_string($login)."', '".mysql_real_escape_string($number)."','".mysql_real_escape_string($gift)."','".md5($password)."');";
        $res = $sql->doQuery($req);
...
function getPronosticsMatch($matchID) {
    ...
    $req = "SELECT * FROM matches WHERE id_match='".mysql_real_escape_string($matchID)."'";
    ...
    $req = "SELECT * FROM pronostics WHERE matches_id_match='".mysql_real_escape_string($matchID)."'";
                ...
                $sql->doQuery(
                    "SELECT * 
                     FROM 
                        pronostics
                     WHERE
                        matches_id_match = '".mysql_real_escape_string($match['id_match'])."' 
                        "
                )
            );
                ...
                $sql->doQuery(
                    "SELECT * 
                     ...
                     WHERE
                        matches_id_match = '".mysql_real_escape_string($match['id_match'])."' 
                    AND    
                        friendlist_name = '".mysql_real_escape_string($friendlist_name)."'   
                        "
                )
            );
...
function getMatchID($team1, $team2, $round) {
    ...
    $req= "SELECT * FROM `matches` WHERE `team1_name`='".mysql_real_escape_string($team1)."' AND `team2_name`='".mysql_real_escape_string($team2)."' AND `round_name`='".mysql_real_escape_string($round)."'";
...
function getLoginFromNumber($number) {
	...
	$req = "SELECT login FROM `members` where number = '".mysql_real_escape_string($number)."'";
