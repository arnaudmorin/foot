...
foreach ($matches as $match){
    ...
    if (   isset($_REQUEST['submitter'])
        ...
        $req = "UPDATE matches
                SET
                    team1_result = '".mysqli_real_escape_string($sql->dbhandle, $_REQUEST['team1_'.$match['id_match']])."',
                    team2_result = '".mysqli_real_escape_string($sql->dbhandle, $_REQUEST['team2_'.$match['id_match']])."',
                    status = '"._PLAYED."'
                WHERE
                    id_match = '".mysqli_real_escape_string($sql->dbhandle, $match['id_match'])."'
                ";
        $res = $sql->doQuery($req);
        if (!$res){
